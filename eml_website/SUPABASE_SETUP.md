...
PowerShell에서 프로젝트 폴더로 이동한 뒤 정적 서버를 실행합니다.
...
- 공개 홈페이지: `http://127.0.0.1:8767/index.html#home`
- 관리자 페이지: `http://127.0.0.1:8767/admin.html`

`file://` 주소로 HTML 파일을 직접 열면 로그인·스토리지·모듈 동작을 정확히 확인할 수 없습니다. 테스트가 끝나면 서버를 실행한 PowerShell에서 `Ctrl+C`를 눌러 종료합니다.

### 2. 설정·로그인 확인

1. 아직 Supabase 값을 넣지 않았다면 관리자 페이지에 **클라우드 설정이 필요합니다**가 표시되는지 확인합니다.
2. Supabase 설정을 마친 뒤 페이지를 새로고침합니다.
3. 아이디에는 `supabase-config.js`의 `adminLoginId`를, 비밀번호에는 Supabase Auth에서 지정한 관리자 비밀번호를 입력합니다.
4. 잘못된 비밀번호로 한 번 시도해 오류 안내가 표시되는지 확인한 뒤 올바른 비밀번호로 로그인합니다.
5. 로그인 후 상단에 **Cloud 연결됨**과 관리자 아이디가 보이는지 확인합니다.

### 3. 콘텐츠 저장 왕복 테스트

운영 콘텐츠를 안전하게 되돌릴 수 있도록 먼저 상단 **Export JSON**을 눌러 백업합니다.

1. **Home**에서 Intro Text 끝에 ` [TEST]`를 붙입니다.
2. **Save & Open Preview**를 눌러 새 창이 열리고 문구가 반영되는지 확인합니다.
3. 공개 홈페이지를 새로고침해 같은 문구가 보이는지 확인합니다.
4. 관리자 페이지로 돌아와 `[TEST]`를 제거하고 **Save Changes**를 누릅니다.
5. 공개 홈페이지를 다시 새로고침해 원래 문구로 복구됐는지 확인합니다.

저장 실패가 표시되면 연속으로 저장하지 말고 오류 메시지를 기록한 뒤 **최신 내용 다시 불러오기**로 현재 클라우드 버전을 확인합니다.

### 4. 갤러리·이미지 테스트

1. **Gallery > Add Gallery Post**로 테스트 게시글을 추가합니다.
2. JPG, PNG 또는 WebP 이미지 두 장을 추가하고 업로드 완료 안내를 기다립니다.
3. 위·아래 버튼 또는 드래그로 이미지 순서를 바꿉니다.
4. Preview에서 대표 이미지가 첫 번째 사진과 일치하는지 확인합니다.
5. 갤러리 카드를 열어 이전·다음 버튼, 키보드 방향키, 배경 클릭, `Esc`, 우측 상단 `X`로 닫기가 모두 동작하는지 확인합니다.
6. 테스트 게시글을 삭제하고 공개 홈페이지에서도 사라졌는지 확인합니다.

### 5. 반응형·세션 테스트

1. 브라우저 개발자 도구의 기기 모드에서 너비 360px, 390px, 768px, 1440px을 차례로 확인합니다.
2. 상단 작업 버튼, 사이드 메뉴, 입력 필드와 이미지 타일이 겹치거나 화면 밖으로 나가지 않는지 확인합니다.
3. 값을 수정하고 저장하지 않은 상태에서 새로고침하거나 로그아웃할 때 경고가 표시되는지 확인합니다.
4. 로그아웃 후 `admin.html`을 다시 열었을 때 편집 화면이 노출되지 않고 로그인 화면으로 돌아오는지 확인합니다.
5. 운영 배포 주소에서도 HTTPS 자물쇠와 로그인·저장·공개 페이지 반영을 한 번 더 확인합니다.

테스트 중 원본을 잃었으면 상단 **Import JSON**으로 처음 내려받은 백업 파일을 불러온 다음 **Save Changes**를 눌러 복구합니다.
